...
function Todolist(props) {
    const todolist = props.todolist.map((task,index) => {
        const taskComplete = task => {
            axios.put(`https://server-todo-6fx4.onrender.com/${task._id}` , {
                _id : task._id,
                todo: task.todo,
        ...
        const removeTask = id => {
            axios.delete(`https://server-todo-6fx4.onrender.com/${id}`).then(res => props.removeTask(res.data)).catch(err => console.log(err))
        } 
        return <li key = {index}>
